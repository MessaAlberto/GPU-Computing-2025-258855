...
#define KERNEL_NO_BOUNDS SpMV_OneThreadPerRow_noBounds
...
#define CUDA_CHECK_KERNEL()                                           \
  do {                                                                              \
    ...
    if (err != cudaSuccess) {                                                       \
      fprintf(stderr, "SpMV_OneThreadPerRow error: %s\n", cudaGetErrorString(err)); \
      return 1;                                                                     \
    }                                                                               \
  } while (0)
...
// ===================== SpMV_OneThreadPerRows =====================
__device__ void SpMV_OneThreadPerRow_body(const int rows, const int *__restrict__ row_ptr,
...
__global__ void SpMV_OneThreadPerRow_noBounds(const int rows, const int *__restrict__ row_ptr,
                                              const int *__restrict__ col_idx,
                                              const double *__restrict__ values,
                                              const double *__restrict__ vec,
                                              double *__restrict__ result) {
  SpMV_OneThreadPerRow_body(rows, row_ptr, col_idx, values, vec, result);
}

template <int minBlocksPerSM>
__global__ __launch_bounds__(BLOCK_DIM, minBlocksPerSM) void SpMV_OneThreadPerRow(
    const int rows, const int *__restrict__ row_ptr, const int *__restrict__ col_idx,
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < WARM_UP; ++i) {
    switch (minBlocksPerSM) {
      case 0:
        KERNEL_NO_BOUNDS<<<gridDim, BLOCK_DIM>>>(KERNEL_PARAMS);
        break;
      case 1:
        KERNEL<1><<<gridDim, BLOCK_DIM>>>(KERNEL_PARAMS);
        break;
      case 2:
        KERNEL<2><<<gridDim, BLOCK_DIM>>>(KERNEL_PARAMS);
        break;
      case 3:
        KERNEL<3><<<gridDim, BLOCK_DIM>>>(KERNEL_PARAMS);
        break;
      default:
        KERNEL<2><<<gridDim, BLOCK_DIM>>>(KERNEL_PARAMS);
        break;
